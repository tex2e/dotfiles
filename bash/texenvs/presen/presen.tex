...
\setbeamertemplate{headline}{ % headerの設定
  \leavevmode\vskip10pt % 上のマージン（狭すぎると見出しがスクリーンからはみ出ることがある）
}
\setbeamertemplate{frametitle}{ % スライドの見出しの設定
  \Large % フォント
  \centering % 中央揃え
  \insertframetitle
  \par\vskip-6pt\color{black}\hrulefill % 見出しの下線
}
...
\setbeamertemplate{footline}{% footerの設定
  ...
  \hspace*{5ex} % 右のマージン
  \vskip15pt % 下のマージン（狭すぎると行番号がスクリーンに映し出されないことがある）
}
...
% \institute{\inst{1} 太郎の所属 \adn \inst{2} 花子の所属}
\date{}
